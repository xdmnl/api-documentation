...
_links | object | See [Body Structure - Links](#introduction/body-structure/links)
...
```
Fetches the information of a teammate. See [resource aliases](#introduction/resource-aliases) to fetch by email.
...
q | object) - Search query. See [Search Parameters](#introduction/search-parameters (optional) | 
...
* `custom` used only through the API (cf [Custom inboxes](#reference/custom-inboxes))
...
```
Fetches the information of an inbox. See [resource aliases](#introduction/resource-aliases) to fetch by address.
...
```
Fetches the information of a contact. See [resource aliases](#introduction/resource-aliases) to fetch by handle.
...
```
Creates a new contact. It might also merge two contacts (see explanation above).
...
When receiving an event from Front (either via a [webhook](#reference/webhooks) or via a [custom inbox](#reference/custom-inboxes) callback), you will receive the preview of the event.
...
<aside class="notice">
  ...
  We recommend the use of `before` search parameter to ensure that the events in each page won't change over the time (see <a href="#introduction/search-parameters/search-criteria-for-events">Search criteria for events</a>).
</aside>
